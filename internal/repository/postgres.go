...
func (r *bookingRepo) Create(ctx context.Context, b *models.Booking) (int64, error) {
	const query = `INSERT INTO bookings (user_id, movie_id, status) VALUES ($1, $2, $3) RETURNING id`
	var id int64
...
func (r *bookingRepo) GetByID(ctx context.Context, id int64) (*models.Booking, error) {
	const query = `SELECT id, user_id, movie_id, status, created_at, updated_at FROM bookings WHERE id = $1`
	b := &models.Booking{}
...
func (r *bookingRepo) GetByUserID(ctx context.Context, userID int64) ([]*models.Booking, error) {
	const query = `SELECT id, user_id, movie_id, status, created_at, updated_at FROM bookings WHERE user_id = $1`
